...
class Config:
    ...
    NUM_EPOCHS = 20
    LEARNING_RATE = 0.001
    NUM_WORKERS = 0 # Количество потоков загрузки данных/как же я мучался с этим УЖАС
...
def create_dataset_metadata(base_dir):
    ...
    for category in tqdm(categories, desc="Индексация изображений"):
        ...
        # Сбор файлов с валидными расширениями
...
# ============= ДОБАВИТЬ В САМОМ НАЧАЛЕ ВАШЕГО СКРИПТА/НОУТБУКА =============
import torch.multiprocessing as mp
import sys

# Устанавливаем метод создания процессов (критично для Windows и Jupyter)
try:
    mp.set_start_method('spawn', force=True)
except RuntimeError:
    pass  # Метод уже был установлен ранее

# ============= ИСПРАВЛЕННЫЙ КОД ДАТАСЕТА И ЗАГРУЗЧИКОВ =============
...
class EfficientAnimalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            # .convert('RGB') спасает от черно-белых (1 канал) или PNG с альфа-каналом (4 канала)
            ...
            # Логируем ошибку, но не прекращаем обучение
...
# pin_memory=True ускоряет перенос данных
...
class AnimalCNN(nn.Module):
    def __init__(self, num_classes):
        ...
        # Вход: 128x128x3 -> Выход: 64x64x32
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1),
            nn.BatchNorm2d(32), # Стабилизирует обучение, позволяет ставить выше LR
            nn.ReLU(),
            nn.MaxPool2d(2, 2) 
        )
        # Блок 2: Текстуры и простые формы
        # Вход: 64x64 -> Выход: 32x32
        ...
        # Блок 3: Сложные паттерны (части тела)
        # Вход: 32x32 -> Выход: 16x16
        ...
        # Блок 4: Высокоуровневые признаки
        # Вход: 16x16 -> Выход: 8x8
        ...
        # Размер входа: 256 каналов * 8 высота * 8 ширина = 16384
        # Если изменишь Config.IMG_SIZE, эту цифру придется пересчитать
        self.fc = nn.Sequential(
            nn.Linear(256 * 8 * 8, 1024),
            nn.BatchNorm1d(1024),
            nn.ReLU(),
            nn.Dropout(0.5), # Отключаем 50% нейронов случайно, чтобы сеть не зубрила
...
# Проверяем, определена ли переменная num_classes (защита от ошибок порядка запуска ячеек)
if 'num_classes' not in locals():
    print("num_classes не найден, вычисляем заново")
...
print("-" * 80)
# Словари для хранения истории (чтобы потом строить графики)
...
for epoch in range(Config.NUM_EPOCHS):
    ...
    # Используем tqdm для визуализации прогресса внутри эпохи
    # leave=False убирает полосу после завершения, чтобы не засорять вывод
    pbar = tqdm(train_loader, desc=f"Ep {epoch+1}", leave=False, unit="batch")
    
    for images, labels in pbar:
        # non_blocking=True позволяет копировать данные асинхронно
        images = images.to(Config.DEVICE, non_blocking=True)
        labels = labels.to(Config.DEVICE, non_blocking=True)
        
        # set_to_none=True чуть быстрее, чем просто zero_grad()
        ...
        # Обновляем цифры в полосе загрузки
    ...
    # 3. Обновление Learning Rate
    # Если определен планировщик, делаем шаг. 
    # Если val_loss не падает, он уменьшит LR.
    ...
    # Инженерная метрика: сколько памяти занято на GPU (в ГБ)
...
# Показывает, насколько уверенно сеть учится. Если Val Loss начинает расти — это переобучение.
plt.subplot(1, 2, 1)
...
# Показывает реальное качество работы.
plt.subplot(1, 2, 2)
...
# Визуальная проверка (Sanity Check)

def imshow(tensor, ax, title=None, color='black'):
    """Вспомогательная функция для отрисовки тензора"""
    # Денормализация: (x * std) + mean. У нас mean=0.5, std=0.5
    img = tensor.cpu().clone()
    img = img * 0.5 + 0.5
    
    # Конвертация в numpy для matplotlib: (C, H, W) -> (H, W, C)
    npimg = img.numpy()
...
# Прогоняем через модель (Inference)
model.eval()
with torch.no_grad():
    # Важно: отправляем на то устройство, где живет модель
...
for i in range(num_samples):
    ...
    # Зеленый цвет текста, если угадали, красный — если нет
...
def save_model_for_production(model, path="model.pth"):
    ...
    # Переводим в режим оценки и на CPU для максимальной совместимости
    model.eval()
    model.cpu()

    # Генерируем фейковый вход нужной размерности для трассировки графа
    # Используем Config.IMG_SIZE, чтобы не хардкодить 128
...
# Запускаем сохранение финальной версии
